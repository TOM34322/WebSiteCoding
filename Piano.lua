...
local infoPage = createTab("Info", 1) -- TAB INFO AJOUTÉE
...
----------------------------------------------------------------
-- INFO TAB CONTENT (POINT ROUGE + TEXTE)
----------------------------------------------------------------
...
infoLayout.Padding = UDim.new(0, 8); infoLayout.SortOrder = Enum.SortOrder.LayoutOrder

local function addSecurityLine(gameName, order)
    local frame = Instance.new("Frame", infoScroll)
    frame.Size = UDim2.new(1, 0, 0, 25); frame.BackgroundTransparency = 1; frame.LayoutOrder = order
    
    local dot = Instance.new("Frame", frame)
    dot.Size = UDim2.new(0, 8, 0, 8); dot.Position = UDim2.new(0, 5, 0.5, -4); dot.BackgroundColor3 = Color3.fromRGB(255, 0, 0); dot.BorderSizePixel = 0
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1, 0)
    
    local txt = Instance.new("TextLabel", frame)
    txt.Size = UDim2.new(1, -25, 1, 0); txt.Position = UDim2.new(0, 25, 0, 0); txt.BackgroundTransparency = 1; txt.Text = gameName; txt.TextColor3 = Color3.new(1,1,1); txt.Font = "GothamSemibold"; txt.TextSize = 14; txt.TextXAlignment = "Left"
end
...
infoDesc.Size = UDim2.new(1, -10, 0, 60); infoDesc.BackgroundTransparency = 1; infoDesc.Text = "Si le script affiche 'Not Found', c'est que la sécurité du jeu bloque la faille. Cherchez d'autres jeux de piano moins connus !"; infoDesc.TextColor3 = Color3.new(0.7, 0.7, 0.7); infoDesc.Font = "Gotham"; infoDesc.TextSize = 12; infoDesc.TextXAlignment = "Left"; infoDesc.TextWrapped = true; infoDesc.LayoutOrder = 5
...
----------------------------------------------------------------
-- PIANO CRASH LOGIC (WITH NOTIFICATIONS)
----------------------------------------------------------------
...
crashBtn.MouseButton1Click:Connect(function()
    ...
    if CrashEnabled then
        Notify("Success", "Faille trouvée ! Crash en cours...", Color3.fromRGB(50, 255, 50))
        task.spawn(function()
...
flingBtn.MouseButton1Click:Connect(function()
    ...
    elseif hrp and hrp:FindFirstChild("PianoFling") then
        hrp.PianoFling:Destroy()
    end
end)

----------------------------------------------------------------
-- PLAYER & OTHERS (INPUTS)
----------------------------------------------------------------
...
toggleBtn.Size = UDim2.new(0, 50, 0, 50); toggleBtn.Position = UDim2.new(0, 15, 0.5, -25); toggleBtn.BackgroundColor3 = Color3.fromRGB(20, 20, 20); toggleBtn.BackgroundTransparency = 1; 
toggleBtn.Text = ""
...
local function addCreditLine(title, info, pos)
    ...
    label.Size = UDim2.new(1, 0, 0, 30); label.Position = UDim2.new(0, 0, 0, pos * 35); label.BackgroundTransparency = 1; label.Text = title .. " :  " .. info; label.TextColor3 = Color3.new(1,1,1); label.Font = "GothamBold"; label.TextSize = 16; label.TextXAlignment = "Left"
...
addCreditLine("Lead Developer", "che3t_2345", 0); 
addCreditLine("Universal", "Piano Crasher Edition", 1); 
addCreditLine("Status", "It depends on the game or security.", 2);
addCreditLine("Version", Version, 3)
